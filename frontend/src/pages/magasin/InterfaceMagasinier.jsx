...
const STATUTS_ACTIFS = ['OUVERT', 'EN_COURS', 'EN_ATTENTE_CORRECTION', 'DEPANNE'];
...
export default function InterfaceMagasinier() {
  ...
  const handleSortie = async (e) => {
    ...
    if (Number(quantite) > Number(pieceSelectionnee.quantiteStock))
      return setErreur(`Stock insuffisant : ${pieceSelectionnee.quantiteStock} ${pieceSelectionnee.unite} disponible(s).`);

    setSubmitting(true);
    try {
      await enregistrerPiece(otSelectionne, pieceSelectionnee.id, quantite);
      setSucces(`✅ Sortie enregistrée : ${quantite} × ${pieceSelectionnee.reference} → ${otCourant?.numero}`);
      setQuantite(''); setTechnicien(''); setPieceSelectionnee(null); setPieceSearch('');
  ...
  return (
    <div className="p-6 text-white">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">Enregistrement des sorties de pièces détachées</p>
      </div>

      {/* Alertes stock */}
      {alertes.length > 0 && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 mb-6">
          <p className="text-red-400 font-medium text-sm mb-2">⚠ {alertes.length} pièce(s) sous le seuil minimum</p>
          <div className="flex flex-wrap gap-2">
            {alertes.slice(0, 5).map(a => (
              <span key={a.id} className="text-xs bg-red-500/20 text-red-300 px-2 py-1 rounded-full font-mono">
          ...
          <form onSubmit={handleSortie} className="space-y-4">
            ...
            <div>
              ...
              {otCourant && (
                <div className="mt-2 p-2 bg-purple-500/10 border border-purple-500/20 rounded-lg text-xs">
                  <span className="text-purple-300 font-mono">{otCourant.numero}</span>
                  <span className="text-gray-400 ml-2">{otCourant.actif_detail?.libelle}</span>
                  <span className={`ml-2 px-1.5 py-0.5 rounded text-xs ${
                    otCourant.priorite === 'critique' ? 'bg-red-500/20 text-red-400' :
                    otCourant.priorite === 'haute'    ? 'bg-orange-500/20 text-orange-400' :
            ...
            <div>
              ...
              <input
                ...
                value={pieceSearch}
                onChange={e => { setPieceSearch(e.target.value); setPieceSelectionnee(null); }}
                className="w-full bg-gray-700 text-white rounded-lg px-3 py-2 text-sm border border-gray-600 outline-none focus:border-purple-500 mb-1"
              />
              {pieceSearch && !pieceSelectionnee && (
                <div className="bg-gray-700 border border-gray-600 rounded-lg max-h-40 overflow-y-auto">
                  {piecesFiltrees.length === 0 ? (
                    <p className="text-gray-500 text-xs p-3 text-center">Aucune pièce trouvée</p>
                  ) : piecesFiltrees.map(p => (
                    <div
                      key={p.id}
                      onClick={() => { setPieceSelectionnee(p); setPieceSearch(p.reference); }}
                    ...
                    >
                      <div>
                        <span className="font-mono text-sm text-purple-300">{p.reference}</span>
                        <span className="text-xs text-gray-400 ml-2">{p.designation}</span>
                        <span className="text-xs text-gray-500 ml-2">📍 {p.emplacement}</span>
                      </div>
                      <span className={`text-xs font-bold ml-2 ${p.est_sous_seuil ? 'text-red-400' : 'text-green-400'}`}>
              ...
              {pieceSelectionnee && (
                <div className="mt-2 p-3 bg-teal-500/10 border border-teal-500/20 rounded-lg">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-mono text-sm text-teal-300">{pieceSelectionnee.reference}</p>
                      <p className="text-xs text-gray-300">{pieceSelectionnee.designation}</p>
                      <p className="text-xs text-gray-500 mt-1">📍 {pieceSelectionnee.emplacement}</p>
                    </div>
                    <div className="text-right">
                      <p className={`text-lg font-bold ${pieceSelectionnee.est_sous_seuil ? 'text-red-400' : 'text-green-400'}`}>
                        {pieceSelectionnee.quantiteStock} {pieceSelectionnee.unite}
                      </p>
                  ...
                  <button
                    type="button"
                    onClick={() => { setPieceSelectionnee(null); setPieceSearch(''); }}
            ...
            <div>
              ...
              <div className="flex gap-2 items-center">
                ...
                />
                {pieceSelectionnee && (
                  <span className="text-gray-400 text-sm">{pieceSelectionnee.unite}</span>
                )}
              </div>
              {pieceSelectionnee && quantite && Number(quantite) > Number(pieceSelectionnee.quantiteStock) && (
                <p className="text-red-400 text-xs mt-1">⚠ Stock insuffisant</p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-gray-700 text-white rounded-lg px-3 py-2 text-sm border border-gray-600 outline-none focus:border-purple-500"
              />
              <p className="text-xs text-gray-500 mt-1">Date/heure et identité magasinier enregistrées automatiquement</p>
            </div>

            <button
              type="submit"
              disabled={submitting || !otSelectionne || !pieceSelectionnee || !quantite}
            ...
            >
              {submitting ? 'Enregistrement...' : '✓ Confirmer la sortie de pièce'}
            </button>
          </form>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-5">
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              ...
              <div className={`rounded-lg p-3 text-center ${alertes.length > 0 ? 'bg-red-500/20' : 'bg-gray-700/50'}`}>
                <p className={`text-2xl font-bold ${alertes.length > 0 ? 'text-red-400' : 'text-white'}`}>{alertes.length}</p>
            ...
            {alertes.length > 0 && (
              <div>
                <p className="text-xs text-red-400 font-medium mb-2">Pièces à réapprovisionner :</p>
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {alertes.map(a => (
                    <div key={a.id} className="flex justify-between items-center text-xs p-2 bg-red-500/10 rounded-lg">
                      <span className="font-mono text-red-300">{a.reference}</span>
                      <span className="text-gray-400 truncate mx-2">{a.designation?.slice(0, 25)}</span>
                      <span className="text-red-400 font-bold whitespace-nowrap">{a.quantiteStock} / {a.seuilMinimum}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-5">
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {ots.length === 0 ? (
                <p className="text-gray-500 text-sm text-center py-4">Aucune intervention active</p>
              ) : ots.map(o => (
                <div
                  key={o.id}
                  onClick={() => { setOtSelectionne(o.id); setOtSearch(''); }}
                ...
                >
                  <div>
                    <p className="font-mono text-sm text-purple-300">{o.numero}</p>
                    <p className="text-xs text-gray-400">{o.actif_detail?.libelle?.slice(0, 35)}</p>
                  </div>
                  <div className="text-right">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      o.priorite === 'critique' ? 'bg-red-500/20 text-red-400' :
                      o.priorite === 'haute'    ? 'bg-orange-500/20 text-orange-400' :
                      'bg-blue-500/20 text-blue-400'
                    }`}>{o.priorite}</span>
                  </div>
                </div>
