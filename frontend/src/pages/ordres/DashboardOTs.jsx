...
const STATUT_CONFIG = {
  OUVERT:                { bg: 'var(--status-blue-bg)',     text: 'var(--status-blue-text)',     label: 'Ouvert' },
  EN_COURS:              { bg: 'var(--status-orange-bg)',   text: 'var(--status-orange-text)',   label: 'En cours' },
  DEPANNE:               { bg: 'var(--status-orange-bg)',   text: 'var(--status-orange-text)',   label: 'Dépanné' },
  EN_ATTENTE_CORRECTION: { bg: 'var(--status-yellow-bg)',   text: 'var(--status-yellow-text)',   label: 'En attente' },
  EN_VALIDATION:         { bg: 'var(--status-purple-bg)',   text: 'var(--status-purple-text)',   label: 'En validation' },
  CLOTURE:               { bg: 'var(--status-green-bg)',    text: 'var(--status-green-text)',    label: 'Clôturé' },
};
      ...
      <div
        ...
        <div
          ...
          <p
            style={{
              fontSize: 12,
              color: data.en_retard > 0 ? 'var(--status-red-text)' : 'var(--text-muted)',
              marginTop: 8,
            }}>
            SLA dépassé
          </p>
          ...
          {data.ots_recents?.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {data.ots_recents?.map(ot => {
                const cfg = STATUT_CONFIG[ot.statut] || STATUT_CONFIG.OUVERT;
                return (
                  <div
                    key={ot.id}
