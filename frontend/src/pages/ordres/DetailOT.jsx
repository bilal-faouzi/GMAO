            ...
            <div className="flex flex-col h-[480px]">
              <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-1">
                {(() => {
                  ...
                  if (vraisCommentaires.length === 0) {
                    return <p className="text-text-muted text-sm text-center py-8">Aucun commentaire</p>;
                  }
                  return vraisCommentaires.map((c) => {
                    const isMe = String(c.utilisateur_detail?.id) === String(user?.id);
                    const initials = c.utilisateur_detail
                      ? `${c.utilisateur_detail.prenom?.[0] || ""}${c.utilisateur_detail.nom?.[0] || ""}`.toUpperCase()
                      : "?";
                    const userName = c.utilisateur_detail
                    ...
                    return (
                      <div key={c.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                        <div className={`flex max-w-[80%] ${isMe ? "flex-row-reverse" : "flex-row"} items-end gap-2`}>
                          {/* Avatar */}
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold border shrink-0 ${isMe ? "bg-primary text-white border-primary" : "bg-primary/10 text-primary border-primary/20"}`}>
                            {initials}
                          </div>
                          {/* Bulle */}
                          <div className={`rounded-2xl px-4 py-2.5 text-sm shadow-sm ${isMe ? "bg-primary text-white rounded-br-sm" : "bg-elevated text-text border border-border-subtle rounded-bl-sm"} ${c.estInterne ? (isMe ? "ring-1 ring-warning" : "border-warning/40 bg-warning-soft") : ""}`}>
                            {/* Nom + date */}
                            <div className={`flex items-center gap-2 mb-1 ${isMe ? "justify-end" : "justify-start"}`}>
                              <span className={`text-[11px] font-semibold ${isMe ? "text-white/90" : "text-text"}`}>
                                {isMe ? "Moi" : userName}
                              </span>
                              {c.estInterne && (
                                <span className="text-[10px] text-warning font-medium">[Interne]</span>
                              )}
                              <span className={`text-[10px] ${isMe ? "text-white/70" : "text-text-muted"}`}>
                                {new Date(c.dateCreation).toLocaleString("fr-FR", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                              </span>
                            </div>
                            {/* Texte */}
                            <p className={`whitespace-pre-wrap ${isMe ? "text-white/95" : "text-text-secondary"}`}>
              ...
              {/* Formulaire d'envoi */}
              <form onSubmit={handleCommentaire} className="flex gap-2 mt-auto pt-2 border-t border-border-subtle">
                <input
                  value={newComment}
                ...
                <button type="submit" className="btn btn-primary px-4 py-2 text-sm font-medium">Envoyer</button>
              </form>
            </div>
          )}
