...
import {
  getOT, changerStatutOT, cloturerOT,
  ajouterCommentaire, getCommentaires, getHistoriqueOT
} from '../../services/ordreService';

const STATUT = {
  OUVERT:                { label:'Ouvert',        cls:'bg-blue-500/20 text-blue-400 border-blue-500/30' },
  EN_COURS:              { label:'En cours',      cls:'bg-amber-500/20 text-amber-400 border-amber-500/30' },
  DEPANNE:               { label:'Dépanné',       cls:'bg-orange-500/20 text-orange-400 border-orange-500/30' },
  EN_ATTENTE_CORRECTION: { label:'En attente',    cls:'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' },
  EN_VALIDATION:         { label:'En validation', cls:'bg-purple-500/20 text-purple-400 border-purple-500/30' },
  CLOTURE:               { label:'Clôturé',       cls:'bg-green-500/20 text-green-400 border-green-500/30' },
};
...
export default function DetailOT() {
  ...
  const charger = async () => {
    try {
      const [o, c, h] = await Promise.all([
        getOT(id), getCommentaires(id), getHistoriqueOT(id)
      ]);
      setOT(o.data);
  ...
  const handleChangerStatut = async () => {
    ...
    setModalStatut(false); setNvStatut(''); setMotif('');
    charger();
  };
  ...
  const handleCommentaire = async (e) => {
    ...
    setNewComment(''); setEstInterne(false);
    charger();
  };
    ...
    <div className="p-6 text-white">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          ...
          <span className={`px-3 py-1 rounded-full text-xs font-medium border ${s?.cls}`}>{s?.label}</span>
          {ot.est_en_retard && <span className="px-2 py-0.5 rounded-full text-xs bg-red-500/20 text-red-400 border border-red-500/30">⚠ Retard</span>}
        </div>
        <div className="flex gap-2">
          {ot.statut !== 'CLOTURE' && <>
            <button onClick={() => { setModalStatut(true); setNvStatut(ot.statut); }}
              className="bg-amber-600 hover:bg-amber-700 px-4 py-2 rounded-lg text-sm transition">Changer statut</button>
            <button onClick={handleCloturer}
              className="bg-green-600 hover:bg-green-700 px-4 py-2 rounded-lg text-sm transition">Clôturer</button>
          </>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="bg-gray-800 rounded-xl p-5 border border-gray-700">
          <h2 className="text-xs text-gray-500 uppercase tracking-wider mb-3">Informations</h2>
          {[
        ...
        <div className="bg-gray-800 rounded-xl p-5 border border-gray-700">
          <h2 className="text-xs text-gray-500 uppercase tracking-wider mb-3">Délais & Coûts</h2>
          {[
            ...
            ['Coût ST',        ot.coutSousTraitance ? `${ot.coutSousTraitance} MAD` : '—'],
          ...
          {[
            ...
            <button key={k} onClick={() => setOnglet(k)}
              className={`py-3 px-4 text-sm font-medium transition border-b-2 -mb-px ${
                onglet===k ? 'text-purple-400 border-purple-400' : 'text-gray-500 border-transparent hover:text-white'}`}>
              {l}
            </button>
          ))}
        </div>
        ...
        <div className="p-4">
          ...
          {onglet==='affectations' && (
            ot.affectations?.length===0
              ? <p className="text-gray-500 text-sm text-center py-8">Aucune affectation</p>
              : <div className="space-y-2">
                  {ot.affectations?.map(a => (
                    <div key={a.id} className="bg-gray-700/40 rounded-lg p-3 flex justify-between items-center">
                      <div>
                        <p className="text-sm font-medium">{a.equipe_detail?.libelle || a.soustraitant_detail?.raisonSociale || '—'}</p>
                        <p className="text-xs text-gray-400">{new Date(a.dateDebut).toLocaleString('fr-FR')}</p>
                      </div>
          ...
          {onglet==='pieces' && (
            <p className="text-gray-500 text-sm text-center py-8">
              {ot.nb_pieces_utilisees===0 ? 'Aucune pièce utilisée' : `${ot.nb_pieces_utilisees} pièce(s) utilisée(s)`}
            </p>
          )}
            ...
            <div>
              <div className="space-y-2 mb-4">
                {commentaires.length===0
                  ? <p className="text-gray-500 text-sm text-center py-4">Aucun commentaire</p>
                  : commentaires.map(c => (
                    <div key={c.id} className={`rounded-lg p-3 ${c.estInterne ? 'bg-amber-500/10 border border-amber-500/20' : 'bg-gray-700/40'}`}>
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-xs font-medium text-gray-300">
                          {c.utilisateur_detail?.prenom} {c.utilisateur_detail?.nom}
                          {c.estInterne && <span className="ml-2 text-amber-400 text-xs">[Interne]</span>}
                        </span>
                        <span className="text-xs text-gray-500">{new Date(c.dateCreation).toLocaleString('fr-FR')}</span>
                      </div>
              ...
              <form onSubmit={handleCommentaire} className="flex gap-2">
                ...
                <button type="submit" className="bg-purple-600 hover:bg-purple-700 px-3 py-2 rounded-lg text-sm transition text-white">
                  Envoyer
                </button>
              </form>
            </div>
          ...
          {onglet==='historique' && (
            historique.length===0
              ...
              : <div className="space-y-2">
                  {historique.map(h => (
                    <div key={h.id} className="flex items-center gap-3 p-3 bg-gray-700/40 rounded-lg">
                      <div className="flex items-center gap-2">
                        {h.ancienStatut && <span className={`px-2 py-0.5 rounded text-xs border ${STATUT[h.ancienStatut]?.cls}`}>{STATUT[h.ancienStatut]?.label}</span>}
                        {h.ancienStatut && <span className="text-gray-500 text-xs">→</span>}
                        <span className={`px-2 py-0.5 rounded text-xs border ${STATUT[h.nouveauStatut]?.cls}`}>{STATUT[h.nouveauStatut]?.label}</span>
                      </div>
                      <div className="flex-1">
                        {h.motif && <p className="text-xs text-gray-400">{h.motif}</p>}
                      </div>
                      <span className="text-xs text-gray-500">{new Date(h.dateChangement).toLocaleString('fr-FR')}</span>
                    </div>
                  ))}
                </div>
          )}
      ...
      {modalStatut && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-2xl border border-gray-700 p-6 w-full max-w-sm">
            ...
            <select value={nvStatut} onChange={e => setNvStatut(e.target.value)}
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button onClick={handleChangerStatut} className="px-4 py-2 text-sm bg-purple-600 hover:bg-purple-700 rounded-lg transition text-white">Confirmer</button>
            </div>
          </div>
        </div>
      )}
