...
import { getOTs, changerStatutOT, cloturerOT } from '../../services/ordreService';
...
export default function ValidationOperateur() {
  ...
  const charger = async () => {
    setLoading(true);
    try {
      const res = await getOTs({ statut: 'EN_VALIDATION' });
      setOTs(res.data.results || res.data);
  ...
  const handleConfirmer = async () => {
    ...
    try {
      if (modal.type === 'ok') {
        await cloturerOT(modal.ot.id, 'corrige', motif || 'Machine confirmée OK par opérateur');
        setMessage('✅ Intervention clôturée. Merci pour votre confirmation.');
      } else {
        await changerStatutOT(modal.ot.id, 'EN_COURS', motif || 'Panne persistante signalée par opérateur');
    ...
    <div className="p-6 text-white max-w-3xl">
      ...
      {message && (
        <div className="bg-green-500/20 border border-green-500/40 text-green-400 rounded-xl p-4 mb-6 text-sm">
          {message}
          <button onClick={() => setMessage('')} className="ml-3 text-green-300 hover:text-white">✕</button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="bg-gray-800 rounded-2xl border border-gray-700 p-12 text-center">
          ...
          <p className="text-gray-300 font-medium">Aucune intervention en attente de validation</p>
        ...
        <div className="space-y-4">
          ...
          {ots.map(ot => (
            <div key={ot.id} className="bg-gray-800 rounded-2xl border border-purple-500/30 p-5">
              {/* Header */}
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-mono text-purple-300 font-semibold">{ot.numero}</span>
                    <span className="text-xs bg-purple-500/20 text-purple-400 border border-purple-500/30 px-2 py-0.5 rounded-full">
                      En validation
                    </span>
                  </div>
                  <p className="text-lg font-medium">{ot.actif_detail?.libelle}</p>
                  <p className="text-sm text-gray-400 font-mono">{ot.actif_detail?.code}</p>
                </div>
                <span className={`text-xs px-2 py-1 rounded-full border font-medium ${
                  ot.priorite === 'critique' ? 'bg-red-500/20 text-red-400 border-red-500/40' :
                  ot.priorite === 'haute'    ? 'bg-orange-500/20 text-orange-400 border-orange-500/40' :
              ...
              <div className="bg-gray-700/30 rounded-xl p-4 mb-4 space-y-2">
                <p className="text-xs text-gray-500 uppercase tracking-wider font-medium mb-2">Résumé de l'intervention</p>
                {ot.description && (
                  <div className="flex gap-2 text-sm">
                    <span className="text-gray-400 min-w-[100px]">Description :</span>
                    <span className="text-gray-300">{ot.description}</span>
                  </div>
                )}
                {ot.dureeReelleMin && (
                  <div className="flex gap-2 text-sm">
                    <span className="text-gray-400 min-w-[100px]">Durée réelle :</span>
                    <span className="text-gray-300">{ot.dureeReelleMin} minutes</span>
                  </div>
                )}
                {ot.affectations?.length > 0 && (
                  <div className="flex gap-2 text-sm">
                    <span className="text-gray-400 min-w-[100px]">Intervenant(s) :</span>
                ...
                {ot.nb_pieces_utilisees > 0 && (
                  <div className="flex gap-2 text-sm">
                    <span className="text-gray-400 min-w-[100px]">Pièces utilisées :</span>
                    <span className="text-gray-300">{ot.nb_pieces_utilisees} pièce(s)</span>
                  </div>
                )}
              </div>
              ...
              <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 mb-4">
                <p className="text-amber-300 font-medium text-sm">
                  🔍 Avez-vous vérifié que l'équipement <strong>{ot.actif_detail?.libelle}</strong> fonctionne correctement ?
                </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => { setModal({ ot, type: 'ok' }); setMotif(''); }}
                  ...
                  <span>✅</span>
                  <span>Machine OK — Clôturer</span>
                </button>
                <button
                  onClick={() => { setModal({ ot, type: 'panne' }); setMotif(''); }}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-2xl border border-gray-700 p-6 w-full max-w-md">
            <h2 className="text-lg font-semibold mb-2">
              {modal.type === 'ok' ? '✅ Confirmer la remise en service' : '❌ Signaler une panne persistante'}
            </h2>
            <p className="text-gray-400 text-sm mb-4">
              {modal.type === 'ok'
                ? `L'OT ${modal.ot.numero} sera clôturé définitivement.`
                : `L'OT ${modal.ot.numero} sera renvoyé en intervention.`}
            </p>
            <textarea
              value={motif}
              onChange={e => setMotif(e.target.value)}
              placeholder={modal.type === 'ok'
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button onClick={handleConfirmer} disabled={submitting}
                ...
                {submitting ? '...' : modal.type === 'ok' ? 'Confirmer OK' : 'Signaler panne'}
              </button>
            </div>
          </div>
        </div>
