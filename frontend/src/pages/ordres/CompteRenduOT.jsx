...
export default function CompteRenduOT() {
    ...
    <button
      ...
      <div className="flex items-center gap-2 text-sm font-semibold text-text">
        ...
        {count !== undefined && count > 0 && (
          <span className="text-xs bg-elevated text-text-muted border border-border-subtle px-1.5 py-0.5 rounded-full">{count}</span>
        )}
      </div>
  ...
  return (
    <div className="page">
      ...
      <div className="hdr">
        <div className="hdr-l">
          <button onClick={() => navigate(-1)} className="text-text-muted hover:text-text text-sm transition block mb-1">
            ← Retour
          </button>
          <h1>Compte rendu d'intervention</h1>
          <p>Documentez l'ordre de travail réalisé sur {ot.actif_detail?.code}</p>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-xl border border-border shadow-card p-5">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <p className="text-[10px] text-text-muted uppercase tracking-wider font-semibold mb-1">Numéro OT</p>
            <p className="text-lg font-mono text-text font-semibold">{ot.numero}</p>
          </div>
          <div>
            <p className="text-[10px] text-text-muted uppercase tracking-wider font-semibold mb-1">Équipement</p>
            <p className="text-sm font-medium text-text">{ot.actif_detail?.code}</p>
          </div>
          <div>
            <p className="text-[10px] text-text-muted uppercase tracking-wider font-semibold mb-1">Priorité</p>
            <p className={`text-sm font-semibold ${
              ot.priorite === "critique" ? "text-danger" : ot.priorite === "haute" ? "text-status-orange" : "text-primary"
            }`}>{ot.priorite}</p>
          </div>
          <div>
            <p className="text-[10px] text-text-muted uppercase tracking-wider font-semibold mb-1">Statut</p>
      ...
      <div className="bg-surface rounded-xl border border-border shadow-card p-5">
        ...
        <div className="bg-elevated rounded-lg p-3 border border-border-subtle">
          <p className="text-sm font-semibold text-text">{ot.actif_detail?.libelle}</p>
          <p className="text-xs text-text-muted font-mono mt-0.5">{ot.actif_detail?.code}</p>
          {di?.actif_detail?.chemin_hierarchique?.length > 0 && (
            <p className="text-[11px] text-text-muted mt-1.5">
              ...
              <span className="text-primary font-medium">{di?.actif_detail?.libelle}</span>
            </p>
          )}
        </div>
        <AnimatePresence>
                  ...
                  <>
                    {optionsAtLevel.map((options, levelIndex) => (
                      <div key={levelIndex}>
                        ...
                        <select value={selectionPath[levelIndex]?.id || ""} onChange={(e) => handleSelectAtLevel(levelIndex, e.target.value)}
                          className="w-full bg-surface text-text rounded-lg px-3 py-2 text-sm border border-border outline-none focus:border-primary">
                          <option value="">Sélectionner...</option>
                          {options.map((a) => <option key={a.id} value={a.id}>{a.code} — {a.libelle}</option>)}
                        </select>
                      </div>
                    ))}
                    {selectionPath.length > 0 && (
                      <div className="flex items-center justify-between pt-2">
                        <p className="text-xs text-text-muted">Sélection : <span className="text-text font-medium">{selectionPath[selectionPath.length - 1]?.code}</span></p>
                        <button onClick={handleChangerActif} disabled={changingActif || selectionPath[selectionPath.length - 1]?.id === ot.actif_detail?.id}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {expandedSections.contexte && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  <div className="bg-surface rounded-xl border border-border-subtle p-4 space-y-3">
                    {di?.titre && (
                      <div>
                        <p className="text-[10px] text-text-muted uppercase tracking-wider font-semibold mb-1">Titre de la demande</p>
                        <p className="text-sm font-medium text-text">{di.titre}</p>
                      </div>
                    )}
                    <div>
                      <p className="text-[10px] text-text-muted uppercase tracking-wider font-semibold mb-1">Problème signalé</p>
                      <p className="text-sm text-text-secondary leading-relaxed">{di?.description || "—"}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div>
                        <span className="text-text-muted">Urgence : </span>
                        <span className={`font-semibold ${di?.urgence === "critique" ? "text-danger" : di?.urgence === "haute" ? "text-status-orange" : "text-primary"}`}>{di?.urgence || "—"}</span>
                      </div>
                      <div>
                        <span className="text-text-muted">Date : </span>
                        <span className="text-text">{formatDate(di?.dateSignalement)}</span>
                      </div>
                      <div>
                        <span className="text-text-muted">Signalé par : </span>
                        <span className="text-text">{di?.signalement_detail ? `${di.signalement_detail.prenom} ${di.signalement_detail.nom}` : "—"}</span>
                      </div>
                      <div>
                    ...
                    {di?.nb_pieces_jointes > 0 && (
                      <div className="pt-3 border-t border-border-subtle">
                        <p className="text-[10px] text-text-muted uppercase tracking-wider font-semibold mb-2 flex items-center gap-1">
                          <Image size={10} /> Pièces jointes ({di.nb_pieces_jointes})
                        </p>
                        <div className="space-y-2">
                          {pjImages.length > 0 && (
                            <div className="grid grid-cols-4 gap-2">
                              {pjImages.map((img) => (
                                <a key={img.id} href={img.url} target="_blank" rel="noopener noreferrer" className="relative rounded-lg overflow-hidden border border-border-subtle bg-elevated">
                          ...
                          {pjAudio.length > 0 && (
                            <div className="space-y-1">
                              {pjAudio.map((a) => (
                                <div key={a.id} className="flex items-center gap-2 bg-elevated rounded-lg px-2 py-1.5 text-xs border border-border-subtle">
                          ...
                          {pjVideo.length > 0 && (
                            <div className="space-y-1">
                              {pjVideo.map((v) => (
                                <div key={v.id} className="bg-elevated rounded-lg p-2 border border-border-subtle">
                          ...
                          {pjOther.length > 0 && (
                            <div className="flex flex-wrap gap-2">
                              {pjOther.map((f) => (
                                <a key={f.id} href={f.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-elevated rounded-lg px-2 py-1 text-xs border border-border-subtle text-text-secondary hover:text-text transition">
                ...
                {expandedSections.pieces && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    <div className="bg-surface rounded-xl border border-border-subtle p-4 space-y-2">
                      {pieces.map((p, i) => (
                        <div key={i} className="flex justify-between items-center bg-elevated rounded-lg px-3 py-2.5 text-sm border border-border-subtle">
                          <div>
                            <p className="text-text font-medium">{p.piece_detail?.reference || p.piece_detail?.libelle || "Pièce"}</p>
                            {p.piece_detail?.description && <p className="text-[11px] text-text-muted">{p.piece_detail.description}</p>}
                          </div>
                          <div className="text-right">
                            <p className="text-text font-semibold">× {p.quantite}</p>
                          </div>
                        </div>
                      ))}
                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    <div className="bg-surface rounded-xl border border-border-subtle p-4 space-y-2">
                      {commentaires.map((c, i) => (
                        <div key={i} className={`bg-elevated rounded-lg px-3 py-2.5 text-sm border ${c.estInterne ? "border-warning/20" : "border-border-subtle"}`}>
                          ...
                          <div className="flex items-center gap-3 mt-1.5 text-[11px] text-text-muted">
                            ...
                            {c.estInterne && <span className="text-warning bg-warning-soft px-1.5 py-0.5 rounded text-[10px] font-medium">Interne</span>}
                          </div>
                        </div>
                      ))}
                    </div>
                        ...
                        <div key={i} className="flex items-start gap-3 text-sm">
                          ...
                          <div className="flex-1">
                            <p className="text-text-secondary">
                              ...
                              <span className="mx-1 text-text-muted">→</span>
                              <span className="text-text font-semibold">{h.nouveauStatut}</span>
                            </p>
                  ...
                  <div className="bg-surface rounded-xl border border-border-subtle p-4">
                    <div className="grid grid-cols-1 gap-2">
                      {di?.signalement_detail && (
                        <div className="bg-elevated rounded-lg p-3 border border-border-subtle">
                          <p className="text-[10px] text-text-muted uppercase tracking-wider font-semibold mb-1">DI créée par</p>
                          <p className="text-sm font-medium text-text">{di.signalement_detail.prenom} {di.signalement_detail.nom}</p>
                          <p className="text-xs text-text-muted mt-0.5">{formatDate(di.dateSignalement)}</p>
                        </div>
                      )}
                      {ot.createur_detail && (
                        <div className="bg-elevated rounded-lg p-3 border border-border-subtle">
                          <p className="text-[10px] text-text-muted uppercase tracking-wider font-semibold mb-1">OT créé par</p>
                          <p className="text-sm font-medium text-text">{ot.createur_detail.prenom} {ot.createur_detail.nom}</p>
                        </div>
                      )}
                      {ot.affectations?.length > 0 && (
                        <div className="bg-elevated rounded-lg p-3 border border-border-subtle">
                          <p className="text-[10px] text-text-muted uppercase tracking-wider font-semibold mb-1">Intervenant(s)</p>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div>
            <h2 className="text-lg font-semibold text-text">Rédiger le compte rendu</h2>
            <p className="text-text-muted text-sm mt-1">Décrivez l'ordre de travail réalisé pour l'historique</p>
          </div>
          ...
          <form onSubmit={handleSubmitRapport} className="space-y-4">
            ...
            <div className="bg-surface rounded-xl border border-border shadow-card p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <button type="button" onClick={() => { setShowActifCorrigeSelector((s) => !s); if (!showActifCorrigeSelector) initCorrigeSelector(); }}
                  className="btn btn-outline" style={{fontSize:'12px',padding:'5px 10px'}}>
                  <Plus size={12} /> {showActifCorrigeSelector ? "Annuler" : "Ajouter un actif"}
                </button>
              </div>
              {(ot?.actifs_corriges?.length > 0 || actifsCorriges.length > 0) && (
                <div className="flex flex-wrap gap-2 mb-3">
                  {ot?.actifs_corriges?.map((ac) => (
                    <span key={ac.id} className="text-xs bg-elevated text-text px-2.5 py-1 rounded-full border border-border-subtle flex items-center gap-1.5">
                      <CheckCircle size={10} className="text-success" />
                      {ac.actif_detail?.code} — {ac.actif_detail?.libelle}
                    </span>
                  ))}
                  {actifsCorriges.map((a) => (
                    <span key={a.id} className="text-xs bg-elevated text-text px-2.5 py-1 rounded-full border border-border-subtle flex items-center gap-1.5">
                      <CheckCircle size={10} className="text-success" />
                      {a.code} — {a.libelle}
                      <button type="button" onClick={() => retirerActifCorrige(a.id)} className="text-text-muted hover:text-danger transition ml-1">
                        <Trash2 size={10} />
                      </button>
                    </span>
                  ))}
                      ...
                      {corrigeLoading ? (
                        ...
                        <>
                          {corrigeOptionsAtLevel.map((options, levelIndex) => (
                            <div key={levelIndex}>
                              <label className="text-[10px] text-text-muted uppercase tracking-wider block mb-1">{levelIndex === 0 ? "Site / Zone" : `Niveau ${levelIndex + 1}`}</label>
                              <select value={corrigeSelectionPath[levelIndex]?.id || ""} onChange={(e) => handleCorrigeSelectAtLevel(levelIndex, e.target.value)}
                                className="w-full bg-surface text-text rounded-lg px-3 py-2 text-sm border border-border outline-none focus:border-primary">
                                <option value="">Sélectionner...</option>
                                {options.map((a) => <option key={a.id} value={a.id}>{a.code} — {a.libelle}</option>)}
                              </select>
                          ...
                          {corrigeSelectionPath.length > 0 && (
                            <div className="flex items-center justify-between pt-2">
                              <p className="text-xs text-text-muted">Sélection : <span className="text-text font-medium">{corrigeSelectionPath[corrigeSelectionPath.length - 1]?.code}</span></p>
                              <button type="button" onClick={ajouterActifCorrige} className="btn btn-success text-xs">
            ...
            <div className="bg-surface rounded-xl border border-border shadow-card p-5">
              ...
              <textarea value={rapport.descriptionTravail} onChange={(e) => setRapport((r) => ({ ...r, descriptionTravail: e.target.value }))}
                placeholder="Détaillez les actions effectuées, les pièces changées, les réglages..." rows={4}
            ...
            <div className="bg-surface rounded-xl border border-border shadow-card p-5">
              ...
              <textarea value={rapport.constatations} onChange={(e) => setRapport((r) => ({ ...r, constatations: e.target.value }))}
                placeholder="État de l'équipement avant/après, observations importantes..." rows={3}
                className="w-full bg-elevated text-text rounded-lg px-3 py-2 text-sm border border-border outline-none focus:border-primary resize-none" />
            </div>
            ...
            <div className="bg-surface rounded-xl border border-border shadow-card p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                {Object.entries(CATEGORIES_CAUSE).map(([k, v]) => (
                  <button key={k} type="button" onClick={() => setRapport((r) => ({ ...r, causeRacine: k }))}
                    className={`py-2 px-3 rounded-lg text-xs font-medium transition border ${
                      rapport.causeRacine === k ? v.color + " border-opacity-100" : "bg-elevated border-border-subtle text-text-muted hover:text-text"
                    }`}>
                    {v.label}
                  </button>
                ))}
            ...
            <div className="bg-surface rounded-xl border border-border shadow-card p-5">
              ...
              <textarea value={rapport.solutionApportee} onChange={(e) => setRapport((r) => ({ ...r, solutionApportee: e.target.value }))}
                placeholder="Résumé de la solution définitive. L'équipement est-il revenu à la normale ?" rows={3}
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => navigate(-1)} className="flex-1 btn btn-ghost justify-center py-3">
                Annuler
              </button>
              <button type="submit" disabled={submitting} className="flex-1 btn btn-primary justify-center py-3">
