...
import { getTypesActifs, createTypeActif, updateTypeActif, deleteTypeActif } from '@/services/actifService'
...
export default function TypeActifsPage() {
  ...
  const fields = [
    ...
    { name: 'libelle',  label: 'Libellé',  placeholder: 'Équipement',  required: true },
  ...
  async function onCreate(form) { await createTypeActif(form); load() }
  async function onEdit(id, form) { await updateTypeActif(id, form); load() }
  async function onDelete(id) { await deleteTypeActif(id); load() }
