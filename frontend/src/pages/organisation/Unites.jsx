...
export default function Unites() {
  ...
  const fields = [
    {
      ...
      options: secteurs.map((s) => ({
        value: s.id,
        label: `${s.site_libelle} / ${s.libelle}`,
      })),
    },
  ...
  const rows = data.map((u) => ({
    id: u.id,
    secteur: (
      <span className="text-text-secondary text-xs">{u.secteur_libelle}</span>
    ),
    code: (
    ...
    productive: (
      <span
        className={`text-xs font-medium ${u.estProductive ? "text-emerald-700 dark:text-emerald-400" : "text-text-muted"}`}>
  ...
  return (
    <CrudPage
      title="Unités"
      description="Gérer les unités de production de l'organisation."
