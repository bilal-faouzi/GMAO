...
const inputCls = "w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500 disabled:opacity-40"
            ...
            <div key={f.name}>
              ...
              {f.type === 'select' ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor={f.name} className="text-sm text-gray-300">{f.checkLabel || f.label}</label>
                </div>
              ) : (
                <input
                  className={inputCls}
...
export default function CrudPage({ title, subtitle, columns, rows, fields, onCreate, onEdit, onDelete, loading }) {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/10">
              {columns.map(c => (
                <th key={c.key} className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">
                  {c.label}
                </th>
              ))}
          ...
          <tbody className="divide-y divide-white/5">
            {loading ? (
              <tr><td colSpan={columns.length + 1} className="px-4 py-10 text-center text-gray-500">Chargement...</td></tr>
            ) : rows.length === 0 ? (
              <tr><td colSpan={columns.length + 1} className="px-4 py-10 text-center text-gray-500">Aucun élément</td></tr>
            ) : (
              rows.map(row => (
                <tr key={row.id} className="hover:bg-white/5 transition-colors">
                  {columns.map(c => (
                    <td key={c.key} className="px-4 py-3 text-gray-300">{row[c.key]}</td>
                  ))}
