      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className={`flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center text-white text-xs transition ${
            error ? "bg-danger-soft cursor-not-allowed" :
            playing ? "bg-primary" : "bg-primary hover:bg-primary-dark"
          }`}>
          {playing ? <Pause size={14} /> : <Play size={14} className="ml-0.5" />}
        </button>
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-text truncate mb-1">{file.nomFichier}</p>
          <div onClick={handleSeek} className="h-1 rounded-full bg-hover cursor-pointer relative overflow-hidden">
            <div className="absolute left-0 top-0 h-full rounded-full bg-primary transition-all"
              style={{ width: `${progress}%` }} />
          </div>
          <div className="flex justify-between mt-1 text-[10px] text-text-muted">
            {error ? <span className="text-danger">Impossible de charger l'audio</span> : <>
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden bg-surface">
      {error ? (
        ...
        <video src={url} controls className="w-full h-36 object-contain bg-surface"
          onError={() => setError(true)} />
      )}
      <div className="px-2 py-1.5 flex items-center justify-between border-t border-border bg-surface">
        <p className="text-[10px] text-text-muted truncate flex-1">{file.nomFichier}</p>
  ...
  return (
    <div className="mt-3 p-3 border border-border rounded-lg bg-elevated/50">
      ...
      <div className="flex items-center flex-wrap gap-1 text-xs">
        ...
        <span className="text-text-muted ml-1 text-[10px]">({actifDetail.code})</span>
      </div>
      {hasChildren && (
        <div className="mt-2">
          <p className="text-[10px] text-text-muted mb-1">Fils directs ({actifDetail.fils.length}):</p>
          <div className="flex flex-wrap gap-1.5">
            {actifDetail.fils.map(f => (
              <span key={f.id} className="px-2 py-0.5 rounded border border-border text-[11px] text-text-secondary bg-surface">
                {f.libelle}
                <span className={`ml-1 text-[9px] px-1 rounded ${
                  f.statut === 'en_panne' ? 'bg-danger-soft text-danger' :
                  f.statut === 'en_maintenance' ? 'bg-warning/20 text-warning' :
